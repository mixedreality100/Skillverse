...
export const AloePage = () => {
  ...
  // Removed shuffleArray function since it's no longer needed
  ...
  // Rest of the useEffect and handler functions remain unchanged
  useEffect(() => {
    const handleResize = () => {
  ...
  const toggleLeavesPopup = () => {
    if (isGelPopupVisible) setIsGelPopupVisible(false);
    setIsLeavesPopupVisible(!isLeavesPopupVisible);
  };

  const handelAboutUsClick = () => navigate("/aboutus");
  const handelExploreCourseClick = () => navigate("/explore");
  ...
  const toggleGelPopup = () => {
    if (isLeavesPopupVisible) setIsLeavesPopupVisible(false);
  ...
  const handleTakeQuiz = () => {
    navigate(`/quiz/${moduleId}`, { state: { fromApp: true } });
  };
  ...
  const handleARClick = () => {
    navigate(`/module-viewer/${moduleId}`, {
      state: { scrollToInteractive: true, fromApp: true },
    });
  };

  const handleVRClick = () => {
    navigate(`/modelvr/${moduleId}`, { state: { scrollToInteractive: true } });
  };
  ...
  if (navigateToCustardApple) return <Navigate to="/custard-apple" />;
        ...
        <div className="main-container">
          ...
          <header className="header w-full h-auto p-4 flex justify-between items-center relative z-10">
            ...
            <div className="nav-buttons hidden md:flex gap-8">
              <NavButton className="transform transition-transform duration-300 hover:scale-110 text-black" onClick={handleCourses}>Courses</NavButton>
              <NavButton className="transform transition-transform duration-300 hover:scale-110 text-black" onClick={handelExploreCourseClick}>Explore</NavButton>
              <NavButton className="transform transition-transform duration-300 hover:scale-110 text-black" onClick={handelAboutUsClick}>About Us</NavButton>
            </div>
            <div className="flex items-center gap-2 sm:gap-4">
              <MusicControl />
              <ProfileButton />
              <button className="mobile-menu-button md:hidden" onClick={toggleMobileMenu} aria-label="Toggle mobile menu">
                {isMobileMenuOpen ? <X size={24} /> : <Menu size={24} />}
              </button>
            </div>
            <div className={`mobile-menu ${isMobileMenuOpen ? "block" : "hidden"} md:hidden bg-white border border-gray-300 rounded-lg shadow-lg absolute right-4 top-16 z-50 w-48`}>
              <button onClick={handleCourses} className="block w-full px-4 py-2 text-left hover:bg-gray-100">Courses</button>
              <button onClick={handelExploreCourseClick} className="block w-full px-4 py-2 text-left hover:bg-gray-100">Explore</button>
              <button onClick={handelAboutUsClick} className="block w-full px-4 py-2 text-left hover:bg-gray-100">About Us</button>
            </div>
          </header>

          {/* Hero Section  */}
          <section className="hero-section relative w-full max-w-7xl mx-auto my-4 sm:my-8 overflow-hidden px-4">
            <img src={moduleDetails.module_image} alt="Hero Image" className="hero-image w-full h-96 object-cover rounded-lg border border-gray-200 shadow-md filter blur-sm" />
            <div className="hero-text-container absolute top-0 left-0 w-full h-full flex flex-col justify-center items-center p-4">
              <div>
                <div className="hero-title text-9xl sm:text-[10rem] md:text-[14rem] lg:text-[16rem] font-black text-green-400 text-center tracking-tight drop-shadow-2xl" style={{ fontFamily: "'Bebas Neue', sans-serif", textShadow: "6px 6px 12px rgba(0, 0, 0, 0.4)", transform: "scale(1.05)", letterSpacing: "-0.02em" }}>
                  {moduleDetails.module_name}
                </div>
                <p className="scientific-name text-xl sm:text-2xl md:text-3xl font-italic text-black text-center mt-2 bg-white bg-opacity-40 py-2 px-2 rounded-lg" style={{ fontFamily: "'Paytone One', sans-serif" }}>
          ...
          <section className="info-cards-section w-full p-4 md:p-8">
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="w-full md:w-1/2 flex justify-center">
                <img src={moduleDetails.module_image} alt="plant image" className="w-64 h-64 sm:w-80 sm:h-80 rounded-lg shadow-lg" />
              </div>
              <div className="w-full md:w-1/2 mt-6 md:mt-0">
                <div className="flex items-center justify-center md:justify-start mb-4 sm:mb-8">
                  ...
                  <h3 className="what-is-heading text-3xl sm:text-4xl md:text-5xl font-bold text-green-800" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                    What is {moduleDetails.module_name}?
                  </h3>
                </div>
                <p className="what-is-description text-base sm:text-lg text-gray-700 px-2 sm:px-0" style={{ fontFamily: "'Poppins', sans-serif" }}>
          ...
          <section className="info-cards-section w-full p-4 md:p-8">
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="w-full md:w-1/2">
                ...
                <div className="space-y-3 sm:space-y-4 px-2 sm:px-0">
                  {[1, 2, 3, 4].map((factId) => (
                    <div
                      key={factId}
                      className={`cursor-pointer p-4 sm:p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all duration-200 hover:shadow-none hover:translate-x-1 hover:translate-y-1 ${!revealedFacts.includes(factId) ? "glow-effect" : ""}`}
                      onClick={() => revealFunFact(factId)}
                    >
                      <div className="flex items-center gap-3 sm:gap-4">
                        <span className="text-2xl sm:text-3xl">{revealedFacts.includes(factId) ? "🌟" : "❓"}</span>
                        <p className="funFact text-sm sm:text-base text-gray-700">
                          {revealedFacts.includes(factId) ? moduleDetails[`funfact${factId}`] : "Tap to reveal fun fact"}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              <div className="w-full md:w-1/2 mt-6 md:mt-0">
                <img src="../src/plantsAssets/Sample.svg" alt="Plant Fun Facts" className="w-full h-auto rounded-lg shadow-lg" />
              </div>
            </div>
            <style jsx global>{`
              @keyframes glow {
                0% { box-shadow: 0 0 5px rgba(34, 197, 94, 0.5), 0 0 10px rgba(34, 197, 94, 0.5), 0 0 15px rgba(34, 197, 94, 0.5); }
                50% { box-shadow: 0 0 10px rgba(34, 197, 94, 0.8), 0 0 20px rgba(34, 197, 94, 0.8), 0 0 30px rgba(34, 197, 94, 0.8); }
                100% { box-shadow: 0 0 5px rgba(34, 197, 94, 0.5), 0 0 10px rgba(34, 197, 94, 0.5), 0 0 15px rgba(34, 197, 94, 0.5); }
              }
              .glow-effect { animation: glow 1.5s infinite ease-in-out; }
            `}</style>
          </section>
            ...
            <div className="space-y-8 sm:space-y-12 px-2 sm:px-4 md:px-8 py-4 sm:py-8">
              ...
              {[1, 2, 3, 4].map((partId) => (
                <div
                  ...
                  role="article"
                  tabIndex={0}
                  aria-label={`Plant part ${moduleDetails[`part${partId}_name`]}`}
                ...
                >
                  <div className="w-full md:w-2/5 overflow-hidden rounded-md relative">
                    <img
                      src={moduleDetails[`part${partId}_image`] || "/placeholder.svg"}
                  ...
                  <div className="w-full md:w-3/5 md:-ml-6 mt-4 md:mt-0">
                    <div className="bg-green-500 rounded-md p-3 sm:p-4 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transform transition-all duration-200 hover:shadow-none hover:translate-x-1 hover:translate-y-1">
                      <p className="text-black whitespace-pre-line">
                        <strong className="text-2xl sm:text-3xl md:text-4xl block mb-2 text-black" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                          <span className="inline-block bg-yellow-400 rounded-full w-8 h-8 sm:w-10 sm:h-10 text-center mr-2 text-black">{partId}</span>
                          {moduleDetails[`part${partId}_name`]}
                        </strong>
                        <br />
                        <span className="text-sm sm:text-base text-black" style={{ fontFamily: "'Poppins', sans-serif" }}>
          ...
          {/* Match the Parts Section (Modified: Removed Randomizer) */}
          <section className="info-cards-section w-full p-4 md:p-8 bg-green-100 rounded-lg">
            <div className="flex flex-col gap-8">
              ...
              <div className="header-section text-center">
                ...
                <p className="text-lg sm:text-xl text-black px-2 sm:px-0 mb-4" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                  Drag and drop the plant parts to their uses. Earn 10 points for each correct match, lose 5 for a wrong one. Match all 4 to win!
                </p>
                <div className="flex flex-col items-center gap-2">
                  <p className="text-xl font-bold text-green-700" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                    Score: <span className="text-3xl">{score}</span>
                  </p>
                  <p className="text-base text-gray-600" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                ...
                {gameCompleted && (
                  <div className="mt-4 p-4 bg-green-100 rounded-md border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-center">
                    <p className="text-2xl font-bold text-black animate-bounce" style={{ fontFamily: "'Paytone One', sans-serif" }}>
              ...
              <div className="matching-area flex flex-col md:flex-row justify-center gap-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {moduleDetails && [
                      ...
                      <div
                        ...
                        onDragStart={(e) => !matchedParts.includes(part) && handleDragStart(e, part)}
                        onKeyDown={(e) => {
                          if ((e.key === "Enter" || e.key === " ") && !matchedParts.includes(part)) {
                            handleDragStart(e, part);
                          }
                        }}
                      >
                        <div className="flex items-center">
                          <Leaf className="text-green-600 mr-2 bg-yellow-400 rounded-full p-2" size={28} />
                          <span className="text-base sm:text-lg font-medium text-black" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                            {part}
                          </span>
                        </div>
                      </div>
                ...
                {/* Right Column: Descriptions (Randomizer Removed) */}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {moduleDetails && [
                      ...
                      <div
                        ...
                        ${matchedParts.some((part) => moduleDetails[`part${matchedParts.indexOf(part) + 1}_description`] === description)
                      ...
                      >
                        <div className="flex items-center">
                          <Flower className="text-green-600 mr-2 bg-yellow-400 rounded-full p-2" size={28} />
                          <span className="text-base sm:text-lg font-medium text-black" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                            {description}
                          </span>
                        </div>
            ...
            {showMatchPopup && (
              <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                <div className="bg-green-100 rounded-md p-6 w-80 md:w-96 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transform transition-all duration-300">
                  <h2 className="text-lg md:text-xl font-bold text-center text-black" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                    {matchMessage}
                  </h2>
            ...
            <div className="w-[90%] h-80 sm:h-96 md:h-[500px] model-container relative">
              {isInstructionVisible && (
                <div className="absolute top-4 left-4 bg-green-100 text-black border-4 border-black rounded-lg p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-10 transform transition-all duration-200 hover:shadow-none hover:translate-x-1 hover:translate-y-1 bounce">
                  <button onClick={() => setIsInstructionVisible(false)} className="absolute top-2 right-2 text-black hover:text-red-500" aria-label="Close instructions">
                    <X size={16} />
                  </button>
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-base sm:text-lg font-bold text-black" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                        How to Interact:
                      </p>
                      <div className="text-sm text-black space-y-1">
                        <div className="flex items-center space-x-2"><span className="text-green-600">🖱️</span><span>Click & drag to rotate</span></div>
                        <div className="flex items-center space-x-2"><span className="text-green-600">🔍</span><span>Scroll to zoom</span></div>
                        <div className="flex items-center space-x-2"><span className="text-green-600">🖱️</span><span>Right-click & drag to pan</span></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-sm sm:text-base" style={{ fontFamily: "'Paytone One', sans-serif" }}>Reset</span>
                </span>
              </button>
              <Canvas camera={{ position: [2.5, 0, -2], fov: 45 }} shadows style={{ width: "100%", height: "100%" }}>
                <ambientLight intensity={2} />
                <directionalLight position={[5, 5, 5]} intensity={1.5} castShadow />
                <directionalLight position={[-5, 5, -5]} intensity={1} />
                <hemisphereLight intensity={1} groundColor="white" />
                {moduleDetails?.glb_file_base64 && <Model glbData={moduleDetails.glb_file_base64} />}
                <OrbitControls ref={controls} enablePan={true} enableZoom={true} enableRotate={true} />
              </Canvas>
              <button
                onClick={() => isSpeaking ? stopSpeech() : speakText(".hero-title, .scientific-name,.what-is-description,.funFact,.parts,.section-header,.benefits-container")}
                className={`p-2 rounded-full shadow-md transition duration-300 fixed bottom-16 sm:bottom-auto sm:top-40 right-4 z-50 ${isSpeaking ? "bg-white text-green-500" : "bg-green-500 text-white hover:bg-green-600"}`}
          ...
          <div className="benefits-container flex flex-wrap justify-center gap-4 sm:gap-8 p-4">
            {[1, 2, 3].map((benefitId) => (
              <div key={benefitId} className="w-full sm:w-5/12 md:w-1/3 mb-4 sm:mb-8 p-2 sm:p-4 transform transition-transform duration-300 hover:scale-105">
                <div className="bg-green-50 rounded-lg p-4 sm:p-6 h-full border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transform transition-all duration-200 hover:shadow-none hover:translate-x-1 hover:translate-y-1 bounce">
                  <div className="flex items-center mb-2 sm:mb-3">
                    ...
                    <h3 className="text-lg sm:text-xl font-semibold text-green-800" style={{ fontFamily: "'Paytone One', sans-serif" }}>
          ...
          <div ref={interactiveSectionRef} id="interactive-experience" className="section-header text-center my-6 sm:my-8">
            <div className="flex items-center justify-center mb-4 sm:mb-8">
              <span className="text-3xl sm:text-4xl mr-2 sm:mr-4"></span>
              <h3 className="text-xl sm:text-2xl md:text-4xl lg:text-6xl font-bold text-black-800" style={{ fontFamily: "'Paytone One', sans-serif" }}>
                🌿Time for some Interactive and Immersive Experience!
              </h3>
            </div>
          </div>
          <div className="ar-vr-container flex flex-wrap justify-center gap-4 sm:gap-8 p-4 bg-green-100 rounded-lg border border-green-200">
            <button onClick={handleARClick} className="ar-vr-icon w-32 h-32 sm:w-48 sm:h-48 bg-green-500 border-4 border-black rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center cursor-pointer transition-all duration-200 hover:shadow-none hover:translate-x-1 hover:translate-y-1 active:scale-95 focus:ring-4 focus:ring-yellow-400 bounce" aria-label="View in Augmented Reality">
              <Flower className="text-black mb-1" size={28} />
              <span className="text-xl sm:text-3xl font-bold text-black" style={{ fontFamily: "'Paytone One', sans-serif" }}>AR</span>
            </button>
            <button onClick={handleVRClick} className="ar-vr-icon w-32 h-32 sm:w-48 sm:h-48 bg-teal-500 border-4 border-black rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center cursor-pointer transition-all duration-200 hover:shadow-none hover:translate-x-1 hover:translate-y-1 active:scale-95 focus:ring-4 focus:ring-yellow-400 bounce" aria-label="View in Virtual Reality">
              <Eye className="text-black mb-1" size={28} />
              <span className="text-xl sm:text-3xl font-bold text-black" style={{ fontFamily: "'Paytone One', sans-serif" }}>VR</span>
            </button>
          ...
          {/* Footer */}
          <footer className="footer w-full bg-cover bg-center mt-8 p-8" style={{ backgroundImage: "url('https://cdn.animaapp.com/projects/66fe7ba2df054d0dfb35274e/releases/676d6d16be8aa405f53530bc/img/hd-wallpaper-anatomy-human-anatomy-1.png')" }}>
            <h2 className="footer-heading text-6xl font-bold text-white text-center">
              Be the one with <span className="text-red-500">Nat</span><span className="text-yellow-400">ur</span><span className="text-red-500">e</span>
            </h2>
            <div className="social-container bg-white rounded-lg p-4 mt-8">
              <div className="social-buttons flex flex-wrap justify-center gap-4">
                <button className="social-button px-4 py-2 bg-white border border-black rounded-full hover:bg-gradient-to-r hover:from-pink-500 hover:via-purple-500 hover:to-yellow-500 hover:text-white hover:scale-105 flex items-center justify-center" onClick={() => window.location.href = "https://www.instagram.com"}>
                  <img src={instagramLogo} alt="Instagram Logo" className="w-9 h-6" />
                </button>
                <button className="social-button px-4 py-2 bg-white border border-black rounded-full hover:bg-black hover:text-white hover:scale-105 flex items-center justify-center" onClick={() => window.location.href = "https://www.twitter.com"}>
                  <img src={twitterLogo} alt="Twitter Logo" className="w-6 h-6" />
                </button>
                <button className="social-button px-4 py-2 bg-white border border-black rounded-full hover:bg-blue-500 hover:text-white hover:scale-105 flex items-center justify-center" onClick={() => window.location.href = "https://www.facebook.com"}>
                  <img src={facebookLogo} alt="Facebook Logo" className="w-6 h-6" />
                </button>
                <button className="social-button px-4 py-2 bg-white border border-black rounded-full hover:bg-red-100 hover:text-white hover:scale-105 flex items-center justify-center" onClick={() => window.location.href = "https://www.pinterest.com"}>
